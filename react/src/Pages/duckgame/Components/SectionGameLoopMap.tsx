...
const challengesAndSolutions = [
  {
    ...
    challenge:
      "Detecting and resolving different types of collisions between objects such as players, bullets, platforms, and spawners (Weapon Generators), providing responses based on the interaction type.",
    solution:
      "The CollisionManager class is used solely for detecting collisions and determining their type using the AABB model. The CollisionEventManager class is used to resolve these collisions, acting and notifying other classes based on the collision type.",
  },
  {
    title: "Weapon Spawner Interaction",
    challenge: "Providing dynamic and random weapons to players on the map.",
    solution:
      "The ItemSpawner class was integrated, which players can interact with to obtain weapons. This class generates random weapons through its interaction with a WeaponFactory, which supplies the weapons to the players.",
  },
  {
    title: "Game Flow Control",
    challenge:
      "Coordinating player actions and their interaction with the map, ensuring synchronized updates of the game state.",
    solution:
      "The GameLoop class was developed, organizing the game cycle in an orderly manner to prevent inconsistencies in the execution of player and object interactions, ensuring a specific order where each event in the game happens.",
  },
  {
    title: "Sending Game State",
    challenge:
      "Communicating the game state and all player statuses to each player.",
    solution:
      "The GameStateMessage data structure was implemented, which contains all the current game state information, including player statuses and game events. This structure compacts all game communication sent by the server to the players.",
  },
];
...
const content = [
  ...
  },
  {
    ...
    content: (
      <DescriptionWithImage
        description='Detects collisions between objects using a small Collider class that contains everything that can interact in the game (players, bullets, platforms, and spawners). It uses the AABB model. It has two ways of determining collisions: simple collision, which reports with a "yes or no" whether a collision occurred, and detailed collision, which generates a struct that, in addition to this, informs where the collision took place.'
        imageSrc={collisionResult}
      />
    ),
  },
  {
    ...
    content: (
      <TextBody>
        Resolves collisions detected by the CollisionManager. It corrects player
        positions if they collide with platforms and informs them if they are
        standing on them. It manages bullet interactions with players, taking
        action depending on the type of bullet (some bullets deal damage, while
        others do not), and handles bullet collisions with platforms, also
        taking action based on the bullet type.
      </TextBody>
    ),
  },
  {
    ...
    content: (
      <>
        <TextBody>
          Defines the order of game actions. This class does not perform any
          actions itself, it simply dictates the order and calls the necessary
          classes to execute them.
        </TextBody>
        <ItemList
          title="Action Order: "
  ...
  },
  {
    ...
    content: (
      <DescriptionWithImage
        description="Contains the updated game state data, which is sent to the players and is what is transmitted through the sockets."
        imageSrc={gameStateMessaage}
      />
    ),
  },
];
